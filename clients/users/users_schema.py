import uuid
from pydantic import BaseModel, Field, EmailStr, ConfigDict


class UserBase(BaseModel):
    """
    Базовые поля пользователя, общие для запросов и ответов.
    """
    model_config = ConfigDict(populate_by_name=True)

    email: EmailStr = Field(..., max_length=250)
    last_name: str = Field(..., min_length=1, max_length=50, alias="lastName")
    first_name: str = Field(..., min_length=1, max_length=50, alias="firstName")
    middle_name: str = Field(..., min_length=1, max_length=50, alias="middleName")


class UserSchema(UserBase):
    """
    Модель данных пользователя.
    """
    id: uuid.UUID


class CreateUserRequestSchema(UserBase):
    """
    Модель запроса на создание пользователя.
    """
    password: str = Field(..., min_length=1, max_length=250)


class CreateUserResponseSchema(BaseModel):
    """
    Модель ответа с данными созданного пользователя.
    """
    user: UserSchema


# Добавили описание структуры ответа получения пользователя
class GetUserResponseSchema(BaseModel):
    """
    Модель ответа получения пользователя.
    """
    user: UserSchema


class UpdateUserRequestSchema(UserBase):
    """
    Модель запроса на обновление пользователя.
    """
    email: EmailStr | None = Field(default=None, max_length=250)
    last_name: str | None = Field(default=None, min_length=1, max_length=50, alias="lastName")
    first_name: str | None = Field(default=None, min_length=1, max_length=50, alias="firstName")
    middle_name: str | None = Field(default=None, min_length=1, max_length=50, alias="middleName")


class UpdateUserResponseSchema(BaseModel):
    """
    Модель ответа обновления пользователя.
    """
    user: UserSchema